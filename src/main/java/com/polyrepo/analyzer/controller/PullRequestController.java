...
public class PullRequestController {
    ...
    public ResponseEntity<Map<String, Object>> getPullRequestNotUpdatedByDays(@PathVariable String orgUserName, @PathVariable int days, @RequestBody RepoNamesList repoNamesList,@RequestHeader("id") String userId) {
        try {
            return new ResponseEntity<>(pullRequestService.getPRNotUpdatedByDays(orgUserName, repoNamesList, days, Integer.parseInt(userId)), HttpStatus.OK);
    ...
    public ResponseEntity<Map<String, Object>> getPullRequestUnMergedByDays(@PathVariable String orgUserName, @PathVariable int days, @RequestBody RepoNamesList repoNamesList,@RequestHeader("id") String userId) {
        try {
            return new ResponseEntity<>(pullRequestService.getUnMergedPullRequestByDays(orgUserName, repoNamesList, days, Integer.parseInt(userId)), HttpStatus.OK);
        } catch (FeignException.Unauthorized e) {
            logger.error(e.getMessage());
            return new ResponseEntity<>(Collections.singletonMap(StringConstants.JSON_MESSAGE_KEY_STRING, StringConstants.JSON_UNAUTHORIZED_VALUE), HttpStatus.UNAUTHORIZED);
        } catch (FeignException.BadRequest | JSONException e) {
